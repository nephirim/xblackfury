...
import { AccountWithProofs, Merkledrop } from '@xblackfury/utils';
import { IncubusClient } from '../dist/client';
import { MsgCreate, MsgClaim } from '../dist/codec/incubus/merkledrop/v1beta1/tx';
import {
  TEST_ADDRESS,
  TEST_MNEMONIC,
  OTHER_TEST_ADDRESS,
...
let api: IncubusClient<typeof modules>;
let apiOther: IncubusClient<typeof modules>;
      ...
      if (signedTxBytes) {
        ...
        if (txRes) {
          ...
          const merkledropIdAttr = logs.findAttribute(
            parsedLogs,
            'incubus.merkledrop.v1beta1.EventCreate',
            'merkledrop_id',
          );
